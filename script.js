...
form.addEventListener('submit', async e => {
    ...
    try {
        ...
        try {
            ...
            if (err.status === 202) {
                const msg = 'Commit activity is being generated by GitHub. Please try again in a few seconds.';
                showError(msg);
                commitActivityDiv.innerHTML = `<p>${msg}</p>`;
            } else {
                const msg = 'Could not fetch commit activity.';
                showError(msg);
                commitActivityDiv.innerHTML = `<p>${msg}</p>`;
            }
        }
